...
def parse_cnf_num_vars(cnf_path: Path):
    # Reads "p cnf N M" header; returns N
...
def main():
    # Usage:
    # python3 versions/asr_v3_active/run_asr_v3.py <input_cnf> <out_prefix> [k_total] [k_step]
    if len(sys.argv) < 3:
        print("Usage: python3 versions/asr_v3_active/run_asr_v3.py <input_cnf> <out_prefix> [k_total] [k_step]")
        print("Example: python3 versions/asr_v3_active/run_asr_v3.py shared/cnf/n120_s2.cnf n120_s2_v3 20 1")
        sys.exit(1)

    input_cnf = Path(sys.argv[1]).expanduser()
    out_prefix = sys.argv[2]
    k_total = int(sys.argv[3]) if len(sys.argv) >= 4 else 20
    ...
    csv_path = res_dir / f"{out_prefix}_asr_v3.csv"
    ...
    def pick_literals_raw(cnf_path: Path, k: int):
    ...
    def pick_new_literals_with_fallback(curr_cnf: Path, want_k: int, used_set):
        # Try to get want_k new literals from picker; if repeats dominate, fall back to unused vars.
        new_lits = []
        attempts = 0
        ...
        while len(new_lits) < want_k:
            attempts += 1
            if attempts > 60:
                # Fallback: pick unused variable IDs in + polarity
                nvars = parse_cnf_num_vars(curr_cnf)
                if nvars is None:
                    die("Could not parse CNF header for fallback (missing 'p cnf N M').")
                ...
                # If still not enough (extremely unlikely), return what we have.
                return new_lits

            # Over-sample candidates to avoid getting stuck
            cand = pick_literals_raw(curr_cnf, want_k * 6)

            for lit in cand:
                # Treat +v and -v as distinct literals; block exact repeats only
    ...
    print("ASR v3 ACTIVE RUN (no-repeat + fallback)")
    ...
    while k_added < k_total:
        ...
        new_lits = pick_new_literals_with_fallback(curr, this_k, used)
