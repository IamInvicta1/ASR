...
def main():
    # Usage:
    # python3 versions/asr_v2_active/run_asr_v2.py <input_cnf> <out_prefix> [k_total] [k_step]
    if len(sys.argv) < 3:
        print("Usage: python3 versions/asr_v2_active/run_asr_v2.py <input_cnf> <out_prefix> [k_total] [k_step]")
        print("Example: python3 versions/asr_v2_active/run_asr_v2.py shared/cnf/n120_s2.cnf n120_s2_v2 10 1")
        sys.exit(1)
    ...
    csv_path = res_dir / f"{out_prefix}_asr_v2.csv"
    ...
    def pick_new_literals(curr_cnf: Path, want_k: int, used_set):
        # Over-sample and filter out previously used literals.
        new_lits = []
        attempts = 0
        ...
        while len(new_lits) < want_k:
            attempts += 1
            if attempts > 60:
                die("Could not find enough NEW literals (too many repeats). Increase k_total, change picker, or add noise.")

            # Over-sample to avoid getting stuck returning the same top literal
    ...
    print("ASR v2 ACTIVE RUN (no-repeat literals)")
    ...
    while k_added < k_total:
        ...
        new_lits = pick_new_literals(curr, this_k, used)
